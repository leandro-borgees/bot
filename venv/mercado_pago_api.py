...
def create_payment_preference(amount, description):
    try:
        preference_data = {
            ...
            "expires": True,
            "expiration_date_to": (datetime.datetime.now() + datetime.timedelta(minutes=15)).strftime("%Y-%m-%dT%H:%M:%S.000-03:00")
        }
...
def create_pix_payment(amount, description):
    try:
        payment_data = {
            ...
            "date_of_expiration": (datetime.datetime.now() + datetime.timedelta(minutes=15)).strftime("%Y-%m-%dT%H:%M:%S.000-03:00")
        }
